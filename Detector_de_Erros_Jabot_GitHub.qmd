...
2.  Marque a opção correspondente à coleção que quer verificar. Para o herbário do IBGE é `IBGE - HERBÁRIO IBGE`
...
Idealmente, este bloco deveria fazer o download do shapefile dos continentes diretamente com a função `rnaturalearth::ne_download()`. Porém, como não funcionou dessa forma, tive que fazer o download manualmente no site do [Natural Earth](https://www.naturalearthdata.com/downloads/) e carregar com a função `sf::st_read()`.
